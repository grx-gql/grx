...
Return **`(, error)`** from resolvers—the executor maps **`error`** strings into **`[core.Error](https://pkg.go.dev/github.com/patrickkabwe/grx/core#Error)`** with stable **`path` segments**. When you need **exact messaging** attach **`&core.Error{ Message:, Path:, … }`** (**[Resolvers → Errors](/concepts/resolvers#errors)** deep dive).
