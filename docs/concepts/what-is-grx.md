...
| **CORS wiring** | Middleware helper (**[`grx.Cors`](https://pkg.go.dev/github.com/patrickkabwe/grx#Cors)**) for browsers and WebSocket origins. |
...
| **WebSocket + SSE transports** | First-class [**subscriptions**](/guides/subscriptions) doc + optional **`pkg/pubsub`** (Redis adapter for multi-node fan-out cases). |
