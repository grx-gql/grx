...
Roughly everything in **HTTP / WebSocket framing** touches **TCP**, **headers**, or **`Match`/`Serve`**. Inside **Transport-agnostic GraphQL** the executor only sees [`core.Request`](https://pkg.go.dev/github.com/patrickkabwe/grx/core#Request) — the intentional split so the same engine runs behind CLI tools, tests, WebSockets, SSE, or a bespoke gateway.
...
[**`pkg/*`**](/reference/) hosts first-party transports (`http`, `websocket`, `sse`, `pubsub`, …).
...
`grx` is a thin **facade**: [`NewServer`](https://pkg.go.dev/github.com/patrickkabwe/grx#NewServer) and options map onto [`server.Config`](https://pkg.go.dev/github.com/patrickkabwe/grx/server#Config).
...
| Phase | What happens |
| ----- | ------------- |
| **Process startup** | Your `schema.Config` (resolver roots + optional enums/unions/etc.) is **reflected once** into a [`schema.Schema`](https://pkg.go.dev/github.com/patrickkabwe/grx/schema#Schema) artifact; an [`exec.Executor`](https://pkg.go.dev/github.com/patrickkabwe/grx/exec#Executor) binds that metadata with plugins and policy hooks; [`server.New`](https://pkg.go.dev/github.com/patrickkabwe/grx/server#New) wraps the executor and registers transports (`pkg/http` is auto-appended on the GraphQL path unless you customise ordering). |
...
- **`GraphQLPath`** (default `/graphql`) — HTTP JSON queries/mutations (`POST`) always land here thanks to the default `pkg/http` transport placed at the tail of that chain unless you reorganise transports.
- **`SubscriptionPath`** — when unset, WebSocket/SSE transports share **`GraphQLPath`** (usual GraphiQL + `graphql-transport-ws` setup). When it **differs**, only concrete bundled `*websocket.Transport` / `*sse.Transport` registrations move — custom transports stay wherever you routed them unless you mimic that split yourself.
