---
title: memory-pubsub
description: API reference for the pubsub package (module path github.com/grx-gql/grx/memory-pubsub).
outline: [2, 4]
lastUpdated: false
---
...
Distributed backends are intentionally kept out of this package so the root module stays dependency\-free; see the github.com/grx-gql/grx/redis-pubsub sub\-module for a production\-ready Redis implementation. Any third\-party broker can be plugged in by satisfying the [PubSub](<#PubSub>) interface  -  the executor and subscription transports never look at the concrete type.
