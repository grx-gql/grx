...
| `pkg/http`           | GraphQL-over-HTTP+JSON transport. Auto-appended by `server.New`; package name `http` (alias as `grxhttp`). |
...
| `pkg/pubsub`         | Publish/subscribe primitive: `PubSub` interface, in-process `Memory`, generic `Typed[T]`, filter helpers.  |
...
1. **Transport match.** `server.ServeHTTP` consults each registered
   ...
   is appended to the chain by `server.New`, so plain `POST /graphql`
