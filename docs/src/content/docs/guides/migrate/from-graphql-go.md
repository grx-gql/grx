...
| `graphql.NewSchema(graphql.SchemaConfig{Query: queryType})`   | `schema.Config{Query: Query{}}` passed to `server.Config.Schema`   |
| `graphql.Do(graphql.Params{Schema: ..., RequestString: ...})` | The HTTP handler returned by `server.New` (or `core.Executor`)     |
...
srv, err := server.New(server.Config{
    Schema:         schema.Config{Query: Query{}},
    PlaygroundPath: "/",
})
...
Then register the WebSocket and SSE transports on `server.Config.Transports`.
