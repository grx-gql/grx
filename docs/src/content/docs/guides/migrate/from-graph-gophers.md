...
| `relay.Handler{Schema: schema}`                                       | `server.New(server.Config{...})` — already an `http.Handler`       |
...
- `server.New` returns an `http.Handler` that already includes the
