...
| `pkg/` | Public, importable building blocks built on top of `core`. Each subpackage owns one concern. |
| `pkg/http/` | `core.Transport` implementation of the canonical GraphQL-over-HTTP+JSON `POST /graphql` wire format. The package is named `http`; consumers that also import `net/http` alias it (e.g. `grxhttp`). |
| `pkg/websocket/` | RFC 6455 WebSocket framing (`conn.go`) plus a `core.Transport` implementation of the `graphql-transport-ws` subprotocol (`dispatcher.go`). Production knobs (init timeout, read/write deadlines, max message size, origin allowlist, `OnConnect` auth hook, server-side ping interval) live on `websocket.Config`. The legacy Apollo `subscriptions-transport-ws` (`graphql-ws`) subprotocol is intentionally not supported. |
| `pkg/sse/` | `core.Transport` implementation of the GraphQL over Server-Sent Events protocol. |
| `pkg/pubsub/` | Pub/Sub interface and in-memory backend used by subscription resolvers for cross-resolver fan-out. The Redis backend lives in the nested `pkg/pubsub/redis` Go module. |
...
| `server/` | HTTP entry point, GraphiQL playground, transport dispatch (`Config.Transports`). Implements `http.Handler`. The default HTTP+JSON transport from `pkg/http` is appended automatically to the chain so plain `POST /graphql` keeps working out of the box. |
...
- **Single module, internal packages by responsibility.** Do not introduce a new top-level package without a clear, distinct responsibility. Prefer extending `core`, `schema`, `exec`, `server`, or `plugin`.
- **`core` has no upward imports.** It must not import `schema`, `exec`, `server`, or `plugin`. Other packages depend on `core`, never the reverse. Sibling packages under `pkg/` (e.g. `pkg/websocket`, `pkg/sse`) may import `core` but follow the same upward-import ban.
- **Every network protocol — including HTTP+JSON — implements `core.Transport`** and lives under `pkg/`. The `server` package wires user-supplied transports in via `Config.Transports` and appends the default `pkg/http` transport at the end of the chain. The `server` package itself only owns playground, favicon, and request routing; it never touches the GraphQL request body.
- **`server` depends on `core`, `pkg/http`, `exec`, `schema`, `plugin`** (and pulls in `pkg/websocket` / `pkg/sse` only when an example or test wires them). Keep GraphQL execution out of transports; transports talk to `core.Executor` only.
...
- Request-level failures (invalid JSON, missing query) are emitted with no `data` field and an HTTP 4xx status. See `pkg/http.Transport.Serve`.
